...
#def indexpull():
    ...
    imagelist=[]
    for x in range(5):
        imagelist.append([temporarylist[x]])
    img1=imagelist[0]
    img2=imagelist[1]
    img3=imagelist[2]
    img4=imagelist[3]
...
#def movieinfo():
    ...
    movieimage=movieimages[infonum]
    movietitle=moviename[infonum]
    print(movieimage,movietitle)
    return jsonify({'movieimage':movieimage,'movietitle':movietitle})

@app.route('/image+info', methods=['POST'])
def movieinfo():
    infonum=int(request.form['infonum'])
    print(infonum)
    movietitle=TMDBname[infonum]
    url = f"https://api.themoviedb.org/3/movie/{movietitle}?api_key={apiKey}"
    response = requests.get(url)
    data=response.json()
